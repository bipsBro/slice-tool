...
async function promptForCredentials(url, existingUserName) {
  console.log({ _token, url, a: existingUserName == _username })
  if (_token) {
...
async function getTempRepo(mainUrl, branch, username, password, token = '') {
  ...
  try {
    ...
    console.log({ repo, origin, mainUrl, a: origin.url() })
    ...
    console.log('error', e);
    console.log({ mainUrl, mainRepoPath, branch })
...
function getTempRepoPath(url) {
  ...
  const tempFilePath = path.join(process.env.PWD, 'git-slice', hash(url).toString())
  if (!nodeFs.existsSync(tempFilePath)) {
    nodeFs.mkdirSync(tempFilePath)
  }
  return tempFilePath
}
